...
import OrderForm from "@/app/shop/[slug]/order-form";

export default async function StudentProductPage({
  params,
}: {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-ink-900 tracking-tight leading-snug">{g.name}</h1>

          <div className="flex items-baseline gap-3 mt-3">
            <span className="text-3xl font-bold text-wood-600 tabular">{g.price}</span>
            {g.priceOld && <span className="text-ink-400 line-through tabular">{g.priceOld}</span>}
          </div>

          {g.soldOut ? (
            <p className="mt-4 rounded-xl bg-ivory-100 border border-navy-100 px-4 py-3 text-ink-600">
              Cây này đang hết hàng. Gọi {prettyPhone()} để bên mình báo khi có lại, hoặc xem mẫu
              khác.
            </p>
          ) : (
            <div className="mt-6">
